...
export default async function CoursePage({
  ...
  const modulesWithState = course.modules.map((mod, index) => {
    ...
    // Downsell plan limit: only the first `moduleLimit` modules are included
    const lockedByPlan = moduleLimit !== null && index >= moduleLimit

    return { ...mod, comingSoon, isLocked, isCompleted, certEligible, certificateId, lockedByPlan }
  })
            ...
            {mod.lockedByPlan ? (
              <div className="flex items-center gap-4 p-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">🔒 Включено в пълния курс</p>
                </div>
                <span className="text-xs text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full flex-shrink-0 font-medium whitespace-nowrap">Пълен курс</span>
              </div>
            ) : mod.comingSoon ? (
              <div className="flex items-center gap-4 p-5">
